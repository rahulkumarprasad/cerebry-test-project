...
class CartSer(serializers.ModelSerializer):
    ...
    def create(self,validated_data):
        ...
        products = validated_data.pop("product_added")
        ...
        for products in products:
            if int(products["product"]["id"]) not in ids_traversed:
                ...
                product_mapper_dict_with_quantity[int(products["product"]["id"])]["pr_cart_obj"].quantity += products["quantity"]
        ...
        if len(pr_ids_to_get) != 0:
            ...
            for pr in products_objs:
                ...
                bulk_create_obj.append(temp_obj)
                cart_product_list.append(temp_obj)

            ProductCart.objects.bulk_create(*bulk_create_obj)
